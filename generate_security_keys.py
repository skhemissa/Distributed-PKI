...
#To do i: add function that verify that the wallet can write permission into the ledger:
#get wallet id command = getaddresses
#then command = listpermissions "publicKeys.write" wallet 
#if result is empty >> no permission
...
def write_DLT(rpcuser, rpcpasswd, rpchost, rpcport, chainName, streamName, unique_id, data, type_key):
	...
	written_data = api_write_ledger.liststreamkeyitems(streamName, unique_id)
        if not written_data:
                print ("Error ! data not written....")
                print ("--")
                print ("")
                sys.exit()
        print written_data 
...
def print_keys(key_list, key_status, key_type):
        ...
        for key in key_list:
                ...
                print ("")
        if (len(key_list) > 0) and (key_status == "Valid"):
                print ("/!\ >> Warning! More then one valid " + key_type +" key.")
                print ("/!\ /!\ >> Please revoke additional keys.")
		sys.exit()

def get_keys_from_ledger(rpcuser, rpcpasswd, rpchost, rpcport, chainName, streamName, holder_id):
...
def global_function(rpcuser, rpcpasswd, rpchost, rpcport, chainName, streamName, key_length):
	...
	while (not reply) :
		reply = raw_input("New holder y/n: ")
	
	if reply in list_no:
		while not ledger_key:
			ledger_key = raw_input("Holder number: ")	
		get_keys_from_ledger(rpcuser, rpcpasswd, rpchost, rpcport, chainName, streamName, ledger_key)
		choose_key_to_generate(rpcuser, rpcpasswd, rpchost, rpcport, chainName, streamName, key_length, ledger_key)
	elif reply in list_yes:
		print ("")
		print ("--")
		print ("new")
		holder_id = random_with_N_digits(random_length)
	        PIN = get_PIN()
        	signature_data, type_signatureKey =  generate_local_data(holder_id, key_length, PIN, "signatureKey")
        	encryption_data, type_encryptionKey = generate_local_data(holder_id, key_length, PIN, "EncryptionKey")
        	write_DLT(rpcuser, rpcpasswd, rpchost, rpcport, chainName, streamName, holder_id, signature_data, type_signatureKey)
        	write_DLT(rpcuser, rpcpasswd, rpchost, rpcport, chainName, streamName, holder_id, encryption_data, type_encryptionKey)
        	print ("holder unique identifier: "+ str(holder_id))
		print ("")
		print ("--")
		print ("")
	else:
